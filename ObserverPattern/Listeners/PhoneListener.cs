...
namespace ObserverPattern.Listeners{
  public class PhoneListener: EventListener
  {
    public PhoneListener(EventManager eventManager){
    ...
    public override void Notify(object arg)
    {
      if (eventManager is VideoData videoData){
        System.Console.WriteLine("Notify all listeners via PHONE with new data" + 
        "\n\tTitle:{0}" +
         "\n\tDescription:{1}" +
